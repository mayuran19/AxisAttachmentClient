...
/**
 * An example of sending an attachment via RPC. This class has a main method
 * that beside the standard arguments allows you to specify an attachment that
 * will be sent to a service which will then send it back.
 * 
 */
public class EchoAttachment {
	 ...
	 * The returned file is compared to the source.
	 * 
	 * @param The
	 *            filename that is the source to send.
	 * @return True if sent and compared.
	 */
	public boolean echo(final boolean doTheDIME, String filename)
			throws Exception {
		...
		call.setOperationName(new QName("EchoAttachmentsService", "echo")); // This
																				// is
																				// the
																				// target
																				// services
	 ...
	 * @return True if sent and compared.
	 */
	public boolean echoDir(final boolean doTheDIME, String filename)
			throws Exception {
		boolean rc = true;

		DataHandler[] attachments = getAttachmentsFromDir(filename); // Get the
																		// attachments
																		// from
																		// the
																		// directory.
		...
		if (attachments.length == 0) {
			throw new java.lang.IllegalArgumentException("The directory \""
					+ filename + "\" has no files to send.");
		}
		...
		call.setOperationName(new QName("urn:EchoAttachmentsService", "echoDir")); // This
																					// is
																					// the
																					// target
																					// services
		...
		call.addParameter("source", XMLType.SOAP_ARRAY, // new
														// XMLType(qnameAttachment),
				ParameterMode.IN); // Add the file.

		call.setReturnType(XMLType.SOAP_ARRAY); // new
												// XMLType(qnameAttachment));
		...
		Object ret = call.invoke(new Object[] { attachments }); // Add the
																// attachment.
		...
		for (i = 0; i < received.length && i < attachments.length; ++i) {
			...
			if (!compareFiles(
					filename + java.io.File.separator + orginalDH.getName(),
					recDH.getName())) {
	...
	public static void main(String args[]) {
		try {
			...
			if (source.isFile()) {
				if (echoattachment.echoUsingSAAJ(argFile)
						&& echoattachment.echo(doTheDIME, argFile)) {
	 ...
	 * @return True if the bytestreams do compare, false for any other reason.
	 */
	...
	/**
	 * This method sends a file as an attachment then receives it as a return.
	 * The returned file is compared to the source. Uses SAAJ API.
	 * 
	 * @param The
	 *            filename that is the source to send.
	 * @return True if sent and compared.
	...
	public boolean echoUsingSAAJ(String filename) throws Exception {
		String endPointURLString = "http://localhost:" + opts.getPort()
				+ "/axis/services/urn:EchoAttachmentsService";
		...
		dataHandlersToAdd.add(new DataHandler(new FileDataSource(new File(
				filename))));
		...
		if (dataHandlersToAdd != null) {
			...
			while (dataHandlerIterator.hasNext()) {
				DataHandler dataHandler = (DataHandler) dataHandlerIterator
						.next();
				javax.xml.soap.SOAPElement element = operation
						.addChildElement(requestEnvelope.createName("source"));
				javax.xml.soap.AttachmentPart attachment = soapMessage
						.createAttachmentPart(dataHandler);
				soapMessage.addAttachmentPart(attachment);
				element.addAttribute(requestEnvelope.createName("href"), "cid:"
						+ attachment.getContentId());
			}
		}
		javax.xml.soap.SOAPMessage returnedSOAPMessage = soapConnection.call(
				soapMessage, endPointURLString);
		...
		// Now lets brute force compare the source attachment
		// to the one we received.
		DataHandler rdh = (DataHandler) ((AttachmentPart) iterator.next())
				.getDataHandler();
